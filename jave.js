    ...
    fetch('http://localhost:3000/animals')  // Adjust the URL to match your setup
      ...
      .then(data => {
        ...
        data.forEach(animal => {
          ...
          clone.querySelector('.species').textContent = `species: ${animal.species}`;
          clone.querySelector('.family').textContent = `family: ${animal.family}`;
          clone.querySelector('.habitat').textContent = `habitat: ${animal.habitat}`;
          clone.querySelector('.found').textContent = `place_of_found: ${animal.place_of_found}`;
          clone.querySelector('.diet').textContent = `diet: ${animal.diet}`;
          clone.querySelector('.description').textContent = `description: ${animal.description}`;
          clone.querySelector('.weight').textContent = `weight_kg: ${animal.weight_kg}`;
          clone.querySelector('.height').textContent = `height_cm: ${animal.height_cm}`;
          clone.querySelector('.card_animal_photo img').src = animal.image;
          clone.querySelector('.card_animal_photo img').alt = `A ${animal.species} named ${animal.name}`;
